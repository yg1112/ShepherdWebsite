...
With local processing simplified, there was room to add something genuinely new: **remote supervision**.

The idea is straightforward. When Shepherd detects a keyword match locally, it can forward a lightweight alert to a cloud endpoint. From there, you get Telegram notifications, a web dashboard, and the ability to send commands back to your machine. This gives you awareness even when you are away from your desk -- which is the entire point of a monitoring tool.

The relay is minimal: a few hundred lines of TypeScript, deployed to the edge, costing essentially nothing to run. It complements the local-first architecture instead of replacing it. Your monitoring data stays ephemeral -- events auto-expire in 24 hours, and no persistent database is involved.
...
本地处理简化后，有了空间去加入真正有价值的新功能：**远程监控**。

思路很直接。当 Shepherd 在本地检测到关键词匹配时，它可以向云端转发一个轻量级的告警。从那里，你可以收到 Telegram 通知、查看 Web 仪表盘，还能向你的机器发送指令。这让你即使不在电脑前也能保持感知——这本来就是监控工具的全部意义。

云端中继非常精简：几百行 TypeScript，部署在边缘节点，运行成本几乎为零。它补充了本地优先的架构，而不是替代它。你的监控数据保持短暂——事件在 24 小时后自动过期，不涉及持久化数据库。
